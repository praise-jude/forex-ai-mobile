...
function ConfigBlock({ account, config }: { account: string; config: ExecutionConfig }) {
  return (
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        Risk &amp; execution config <Text style={styles.titleHint}>(env vars — see README)</Text>
      </Text>
...
const styles = StyleSheet.create({
  ...
  titleHint: { textTransform: "none", fontWeight: "400", color: DashboardColors.textMuted },
  block: { borderRadius: 12, borderWidth: 1, borderColor: DashboardColors.border, backgroundColor: DashboardColors.surfaceAlt, padding: 12, gap: 2 },
  blockTitle: { fontSize: 11, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.4, color: DashboardColors.textMuted, marginBottom: 4 },
  row: {
    flexDirection: "row",
    alignItems: "baseline",
  ...
  value: { fontSize: 12, fontWeight: "700", color: DashboardColors.textPrimary, fontVariant: ["tabular-nums"] },
});
